...
for song in album:
    ...
    for frame_length in all_frames_lengths:
        ...
        pca_mfcc_signal = np.linalg.norm(pca.fit_transform(scaler.fit_transform(mfcc_features)), axis=1)
        ...
        for detector in all_detectors:
            ...
            plt.plot(time_axis, pca_mfcc_signal, label=f"PCA 0-3")
            ...
            plt.legend()
            plt.grid()
            directory = f"./plots/mfcc_pca_normalized_0-3/{detector_name}/{frame_length}/"
            if not os.path.isdir(directory):
                 os.makedirs(directory)
            plt.savefig(f"{directory}/{song["title"]}_mfcc_pca_0-3_{detector_name}_{frame_length}.png")
