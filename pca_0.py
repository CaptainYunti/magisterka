...
for song in album:
    ...
    for frame_length in all_frames_lengths:
        ...
        for detector in all_detectors:
            ...
            plt.plot(time_axis, pca_mfcc_signal, label=f"PCA 0")
            ...
            plt.legend()
            plt.grid()
            directory = f"./plots/mfcc_pca_normalized/{detector_name}/{frame_length}/"
            if not os.path.isdir(directory):
                 os.makedirs(directory)
            plt.savefig(f"{directory}/{song["title"]}_mfcc_pca_{detector_name}_{frame_length}.png")
